...
///
/// Segments should already be clipped to the polygon. Only cells
/// that contain trail segments will be marked as active.
pub fn compute_grid(
    segments: &[Segment],
    ...
    let cols = ((bbox.max().x - bbox.min().x) / dlon).ceil() as usize;
    let rows = ((bbox.max().y - bbox.min().y) / dlat).ceil() as usize;

    let config = GridConfig {
        cell_size_m,
        origin_lon: bbox.min().x,
        origin_lat: bbox.min().y,
        dlat,
        dlon,
        cols,
        rows,
    ...
    for (seg_idx, seg) in segments.iter().enumerate() {
        ...
        let points = discretize(&seg.geometry, DISCRETIZE_STEP_M, true);

        let mut seen_cells = std::collections::HashSet::new();

        for (lat, lon) in &points {
            if let Some(cell_id) = point_to_cell(*lat, *lon, &config, bbox) {
...
/// Compute grid config for a bbox + cell_size (no segments, no coverage).
/// Used for live preview when drawing a polygon.
pub fn compute_preview_cells(cell_size_m: f64, bbox: &Rect<f64>) -> GridConfig {
    let center_lat = bbox.center().y;
    let dlat = cell_size_m / EARTH_RADIUS_M * (180.0 / std::f64::consts::PI);
    let dlon = cell_size_m / (EARTH_RADIUS_M * center_lat.to_radians().cos())
        * (180.0 / std::f64::consts::PI);
...
fn point_to_cell(lat: f64, lon: f64, config: &GridConfig, bbox: &Rect<f64>) -> Option<usize> {
    if lat < bbox.min().y || lat > bbox.max().y || lon < bbox.min().x || lon > bbox.max().x {
        return None;
    }
